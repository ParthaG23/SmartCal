...
import {
  ...
  FiAlertCircle, FiPhone, FiArrowRight, FiChevronDown,
} from "react-icons/fi";
...
export default function Login() {
  ...
  const getFullPhone = () => {
    const digits = phoneNum.replace(/\D/g, ""); // strip non-digits
  ...
  const handleGoogle = async () => {
    ...
    } catch (err) { onError(err, "Google login failed."); }
  ...
  const handleEmail = async (e) => {
    ...
    try {
      ...
      navigate(from, { replace: true });
    } catch (err) { onError(err, "Invalid email or password."); }
  ...
  const handleSendOTP = async () => {
    ...
    try {
      const conf = await sendOTP(fullPhone); // ✅ always E.164 format
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      login({ token, ...user });
      navigate(from, { replace: true });
    } catch (err) { onError(err, "Invalid OTP. Please try again."); }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center
      ...
      <motion.div
        ...
        className="relative w-full max-w-4xl min-h-[580px] rounded-3xl overflow-hidden shadow-2xl
      ...
      >
          ...
          <div className="relative z-10 flex flex-col items-center gap-5">
            ...
            <div>
              ...
              <p className="text-white/65 text-sm leading-relaxed max-w-[220px]">
                Don't have an account yet? Sign up and start calculating smarter.
              </p>
            </div>
            <Link to="/signup"
        ...
        <div className="flex-1 flex flex-col justify-center px-8 py-10 lg:px-12">
          ...
          <p className="text-sm mb-6 text-gray-400 dark:text-white/40">Login to your CalcVision account</p>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-2.5 px-4
              ...
              hover:border-gray-300 dark:hover:border-white/20">
            <FiPhone size={16} />
            {tab === "phone" ? "Use Email instead" : "Continue with Phone"}
          </button>
                    ...
                    <motion.div key="verify" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                      ...
                      <button onClick={handleVerifyOTP} disabled={anyLoading}
                        ...
                        {loadingMap.otp
                          ...
                          : <>Verify OTP <FiArrowRight size={14} /></>
                        }
                      </button>
                    </motion.div>
                  )}
